...
fetch('/data')
  ...
  .then(data => {
    products = data; // Save the retrieved data in the 'products' array
    console.log(products); // Print the array to verify
    // Continue with any further processing or rendering of the data
   initApp();
  })
  .catch(error => {
...
function reloadCard(){
    ...
    listCards.forEach((value, key)=>{
        ...
        if(value != null){
            ...
            newDiv.innerHTML = `
                <div><img src="/dPicture?id=${value.id}"/></div>
                <div>${value.name}</div>
  ...
  listCards.forEach(function (value) {
    if (value != null) {
		productsnameInput.value += value.name + ' (' + value.quantity + ') '+ ', ';
      productsnameDiv.innerHTML += `<div>${value.name}(${value.quantity})</div>`;
    }
  });
